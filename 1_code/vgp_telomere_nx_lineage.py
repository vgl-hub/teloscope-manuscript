#!/usr/bin/env python3
"""
Telomere Nx plots for VGP assemblies – per-run workflow (v11).

Changes
--------------
- Adjusted to git repo.
"""
...
run_label = "v11"  # Label for output filenames

vgp_metadata_file = os.path.join(data_dir, "25.09.25_metadata_telomere_assembly_simplified.csv")
vgp_terminal_telomeres = os.path.join(data_dir, "25.12.05_compiled_TTAGGG_terminal_telomeres_filtered.bed")
...
df = pd.read_csv(
    ...
    names=[
        ...
        "distance2end", "distance2end_pct", "chr_position", "chr_position_pct", "canonical_pct"
    ]
)
...
# ───────────────────────────────────────────────────────────────────────────
# Compute lineage-level N50 metrics and write back to vgp_metadata_file
# ───────────────────────────────────────────────────────────────────────────
...
# Map each Accession in vgp_metadata_file to lineage from current BED (if present)
...
tech_df.to_csv(vgp_metadata_file, index=False)
...
for lin in lineage_order:
    ...
    # ax.step(perc_steps, mean_curve, where="mid", color=lineage_colors[lin], linewidth=1.2, label=f"{lin} (n={n})")
    ax.step(perc_steps, mean_curve, where="mid", color=lineage_colors[lin], linewidth=1.2, label=lin)
ax.set_xlabel("Nx (%)", fontsize=9)
ax.set_ylabel("Telomere length (Kbp)", fontsize=9)
ax.tick_params(axis='both', labelsize=8)
ax.set_xlim(0, 101)
ax.legend(title="Lineage", fontsize=6, title_fontsize=7)
for spine in ax.spines.values():
    spine.set_linewidth(0.6)
fig.tight_layout()
for ext in ["png", "pdf"]:
    fig.savefig(
        os.path.join(figures_dir, f"Telomere_Nx_mean_by_lineage_{run_label}.{ext}"),
...
for lin in lineage_order:
    ...
    # ax.step(perc_steps, median_curve, where="mid", color=lineage_colors[lin], linewidth=1.2, label=f"{lin} (n={n})")
    ax.step(perc_steps, median_curve, where="mid", color=lineage_colors[lin], linewidth=1.2, label=lin)
ax.set_xlabel("Nx (%)", fontsize=9)
ax.set_ylabel("Telomere length (Kbp)", fontsize=9)
ax.tick_params(axis='both', labelsize=8)
ax.set_xlim(0, 101)
...
for ext in ["png", "pdf"]:
    fig.savefig(
        os.path.join(figures_dir, f"Telomere_Nx_median_by_lineage_{run_label}.{ext}"),
        dpi=600 if ext == "png" else None
    )
plt.close(fig)
...
for ext in ["png", "pdf"]:
    fig.savefig(
        os.path.join(figures_dir, f"Telomere_Nx_multipanel_mean_by_lineage_{run_label}.{ext}"),
        dpi=600 if ext == "png" else None
    )
plt.close(fig)
...
for ext in ["png", "pdf"]:
    fig.savefig(
        os.path.join(figures_dir, f"Telomere_Nx_multipanel_median_by_lineage_{run_label}.{ext}"),
        dpi=600 if ext == "png" else None
    )
plt.close(fig)
print(f"[OK] Plots + metadata update for run {run_label} saved to {figures_dir} and {vgp_metadata_file}")